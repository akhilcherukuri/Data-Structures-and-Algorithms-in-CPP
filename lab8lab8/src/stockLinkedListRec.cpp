...
void SingleLinkedList::DisplayLinkedList(int disp){
	if(head==NULL){
		...
		if(disp==1){
			...
			if(disp==2){
				cout<<"List of all the stocks in Reverse order:"<<endl;
				DisplayReverseR(current);
			}
	}
}

void SingleLinkedList::DisplayForwardR(stockNode* current){
	if(current!=NULL){
		cout<<current->s;
...
void SingleLinkedList::DisplayReverseR(stockNode* current){
	if(current!=NULL){
		DisplayForwardR(current->link);
		cout<<current->s;
	}
